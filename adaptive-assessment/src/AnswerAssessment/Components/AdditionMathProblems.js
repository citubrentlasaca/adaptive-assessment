...
var data = [
    ...
    useEffect(() => {
    const easyRows = data.filter(row => row[2] === "Easy");
    const randomIndex = Math.floor(Math.random() * easyRows.length);
    setRandomRow(easyRows[randomIndex]);
    }, []);
    ...
    const handleSubmitAnswer = () => {
        ...
        setCorrect(answer === String(randomRow[1])); // Check if the input is the same as the answer
        setAnswerTime(timer); // Store the value of the timer
    ...
    if (randomRow) {
        axios
        .post('https://localhost:11964/predict', {
            problem: randomRow[0],
